...
pattern.y_num = 9; 		% frames of Y, number of different spatial frequencies
...
% 4 pixel period; spat. freq. 15 degs/cycle
Pats(:, :, 1, 1) = repmat([ones(4,2) zeros(4,2)], 1, 24); 
% 6 pixel period; spat. freq. 22.5 degs/cycle
Pats(:, :, 1, 2) = repmat([ones(4,3) zeros(4,3)], 1, 16); 
...
Pats(:, :, 1, 5) = repmat([ones(4,8) zeros(4,8)], 1, 6); 
% 24 pixel period; spat. freq. 90 degs/cycle
Pats(:, :, 1, 6) = repmat([ones(4,12) zeros(4,12)], 1, 4);
% 32 pixel period; spat. freq. 120 degs/cycle
Pats(:, :, 1, 7) = repmat([ones(4,16) zeros(4,16)], 1, 3);
% 48 pixel period; spat. freq. 180 degs/cycle
Pats(:, :, 1, 8) = repmat([ones(4,24) zeros(4,24)], 1, 2);
% 96 pixel period; spat. freq. 360 degs/cycle
Pats(:, :, 1, 9) = [ones(4,48) zeros(4,48)];
