...
LminusR_trials =                NaN(size(start_inds)); % initialize
...
for trial = 1:length(start_inds)
    ...
    if min(wb_freq(trial_inds)) > WB_THRESH % didn't stop flying
        LminusR_trials(trial) = mean(left_amp(trial_inds) - right_amp(trial_inds));
    end
end
...
LminusR =               NaN(length(SPEEDS)-1,length(WAVELENGTHS)-1); % initialize
...
for i = 1:length(SPEEDS)-1
    for j = 1:length(WAVELENGTHS)-1
        ...
        LminusR(i,j) = nanmean(LminusR_trials(stimulus_speed_trials == stimulus_speed(i,j) & stimulus_wavelength_trials == stimulus_wavelength(i,j)));
        num_trials(i,j) = sum(~isnan(LminusR_trials(stimulus_speed_trials == stimulus_speed(i,j) & stimulus_wavelength_trials == stimulus_wavelength(i,j))));
    end
end